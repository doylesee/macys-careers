...
function ds_tm_get_jobs_ajax(url){
  ...
  var t = $("#jobs-search").offset().top - 200;
  t = t > 0 ? t : 1;
  ...
  $.get(url, function(data) {
    ...
    document.title = (tagtitle!="") ? tagtitle : "Macy's Careers";
...
$(document).ready(function() {
    ...
    // uncomment for talemetry environment $(document).on("click", ".facet-item__option-link", function(e){ ds_tm_facet_click(e); }); 
...
$(document).ready(function() {
  ...
  // Career Area pages
  if ( ($('#career-area-featured-openings--actual').length > 0) && ($('#career-area-featured-openings--page').length > 0) ) {
    $('#career-area-featured-openings--page').html($('#career-area-featured-openings--actual').html());
  }

  // Job Details page
  if ( $('.job-details__description').length != 0 ) {
    // Remove all empty DIVs
    $('.job-details__description div').each(function() {
